...
extensions = [
    ...
    # Use our own version of 'IPython.sphinxext.ipython_directive'
    # to capture any logging output
...
#
# Is this still relevant now using astropy_helpers? It doesn't appear
# to be.
#
# numpydoc_show_class_members = False

# Place the autogenerated images in their own directory (which is
# stored in the git repository so that it already exists) rather than
# the default (_static/), so that it is easy to use .gitignore to
# hide them from git status. Note that this does not work with
# IPython 4.1 or early versions of 4.2 - see
...
latex_documents = [
  (master_doc, 'Sherpa.tex', u'Sherpa Documentation',
   u'Chandra X-ray Center, Smithsonian Astrophysical Observatory', 'manual'),
]
